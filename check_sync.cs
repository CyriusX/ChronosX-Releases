...
using (var reader = errorsCmd.ExecuteReader())
{
    while (reader.Read())
    {
        Console.WriteLine($"{reader["entity_type"]}: {reader["last_error"]} (attempts: {reader["attempt_count"]})");
    }
}
...
using (var reader = todayCmd.ExecuteReader())
{
    while (reader.Read())
    {
        ...
        Console.WriteLine($"{reader["app_display_name"]}: {total / 3600}h {(total % 3600) / 60}m");
    }
}

// Check activity sessions count by day
var dailyCmd = connection.CreateCommand();
dailyCmd.CommandText = @"
    SELECT DATE(started_at) as day, COUNT(*) as count, SUM(duration_seconds) as total
    FROM activity_sessions
...
using (var reader = dailyCmd.ExecuteReader())
{
    ...
    {
        var total = Convert.ToInt64(reader["total"]);
        Console.WriteLine($"{reader["day"]}: {reader["count"]} sessions, {total / 3600}h {(total % 3600) / 60}m");
    }
}
