...
namespace TimeTrack.MacOSAgentService.RemoteCommands;

/// <summary>
/// macOS implementation of IRemoteCommandExecutor.
/// Handles remote commands pushed from the backend admin panel.
/// </summary>
public sealed class MacOSRemoteCommandExecutor : IRemoteCommandExecutor
{
        ...
        try
        {
            ...
            if (_ipcServer.IsClientConnected)
            {
                await _ipcServer.SendEventAsync(new IpcEvent
                {
                    ...
                    Payload = new { title = payload.Title, body = payload.Body ?? "", kind = "generic", tag = "remote-notification" }
                }, ct);
            }
    ...
    {
        _logger.LogWarning("Agent restart requested by remote admin. Shutting down...");
    ...
    private async Task<CommandResult> ExecuteKanbanNotificationAsync(string kind, string? payloadJson, CancellationToken ct)
    {
        ...
        try
        {
            ...
            var (title, body) = kind switch
            {
                "task_assigned" => ("Nova tarefa atribuída",
                    !string.IsNullOrEmpty(projectName) && !string.IsNullOrEmpty(taskTitle)
                        ? $"{projectName} · {taskTitle}" : taskTitle ?? "Você tem uma nova tarefa"),
                "task_unassigned" => ("Tarefa removida",
                    !string.IsNullOrEmpty(projectName) && !string.IsNullOrEmpty(taskTitle)
                        ? $"{projectName} · {taskTitle}" : taskTitle ?? "Tarefa foi reatribuída"),
                "task_updated" => ("Tarefa atualizada",
                    !string.IsNullOrEmpty(projectName) && !string.IsNullOrEmpty(taskTitle)
                        ? $"{projectName} · {taskTitle}" : taskTitle ?? "Detalhes atualizados"),
                "project_membership_changed" => ("Participação em projeto", projectName ?? "Seus acessos mudaram"),
                _ => ("Notificação", "")
            };
            ...
            if (_ipcServer.IsClientConnected)
            {
                ...
                {
                    EventType = "showNotification",
                    Payload = new { title, body, kind, tag = $"kanban-{kind}-{taskId ?? projectId ?? Guid.NewGuid().ToString()}", taskId, projectId, projectColor }
                }, ct);
