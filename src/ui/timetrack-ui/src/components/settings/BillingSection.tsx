...
import { CreditCard, ExternalLink, Check, AlertTriangle, Loader2, Crown, Users, Monitor, FileText, Download, X, Minus, Plus } from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';
import { useTranslation } from 'react-i18next';
import { usePermissions } from '../../hooks/usePermissions';
import { useNotifications } from '../../stores/uiStore';
import { getSubscriptionStatus, listPlans, createCheckout, createPortal, cancelSubscription, listInvoices, updateSeats } from '../../services/billingApi';
import type { SubscriptionStatusResponse, PlanResponse, SubscriptionStatus, InvoiceResponse } from '../../types/billing';
...
export function BillingSection() {
  ...
  const handleUpdateSeats = async () => {
    ...
    try {
      const result = await updateSeats(seatQuantity);
      if (result.prorationType === 'full_refund') {
      ...
      const msg = error?.response?.data?.errors?.newQuantity?.[0]
        || error?.response?.data?.message
        || t('billing.seatsError');
      notify.error(msg);
    } finally {
  ...
  if (isLoading) {
    return (
      <div className="space-y-6">
        <div>
          <h2 className="text-[20px] font-semibold text-[#f5f7fb]">{t('billing.title')}</h2>
          <p className="text-[13px] text-[rgba(245,247,251,0.5)] mt-1">
            {t('billing.subtitle')}
          </p>
        </div>
        <div className="animate-pulse space-y-4">
          <div className="h-32 bg-[rgba(26,29,46,0.6)] rounded-2xl" />
          <div className="h-48 bg-[rgba(26,29,46,0.6)] rounded-2xl" />
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-8">
      ...
      <div>
        <div className="flex items-center gap-3 mb-1">
          <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-[#8b5aff] to-[#6366f1] flex items-center justify-center">
      ...
      <div className={`bg-gradient-to-br from-[rgba(26,29,46,0.8)] to-[rgba(17,19,28,0.8)] border rounded-2xl p-6 ${
        ...
        {subscription && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
            ...
            <UsageStat
              icon={Monitor}
              label={t('billing.devices')}
              current={subscription.currentDevices}
              max={subscription.quantity}
            />
          </div>
        )}

        {/* Seat management for active subscriptions */}
          ...
          <div className="flex items-start gap-3 p-3 rounded-xl bg-[rgba(245,158,11,0.08)] border border-[rgba(245,158,11,0.15)] mb-4">
            ...
            <div>
              ...
              <p className="text-[12px] text-[rgba(245,247,251,0.5)] mt-0.5">
                {t('billing.pastDueDesc', { date: formatDate(subscription.gracePeriodEnd) })}
              </p>
            </div>
          </div>
        )}
          ...
          >
            ...
            >
              ...
              <div className="flex items-center gap-3 justify-end">
                <button
                  onClick={() => setShowCancelConfirm(false)}
                  className="px-4 py-2.5 rounded-xl text-[13px] font-medium text-[rgba(245,247,251,0.6)] hover:text-[#f5f7fb] transition-colors"
                >
                  {t('billing.back')}
                </button>
                <button
      ...
      {invoices.length > 0 && (
        <div className="space-y-4">
          <div className="flex items-center gap-3 mb-1">
            <div className="w-8 h-8 rounded-lg bg-[rgba(26,29,46,0.8)] flex items-center justify-center">
              <FileText className="w-4 h-4 text-[rgba(245,247,251,0.4)]" />
            </div>
            <h3 className="text-[16px] font-medium text-[rgba(245,247,251,0.7)]">{t('billing.invoiceHistory')}</h3>
          </div>
                  ...
                  {invoices.map((invoice) => (
                    <tr key={invoice.id} className="border-b border-[rgba(255,255,255,0.03)] last:border-b-0 hover:bg-[rgba(255,255,255,0.02)] transition-colors">
                      ...
                      <td className="px-5 py-3.5 text-center">
                        <InvoiceStatusBadge status={invoice.status} refundStatus={invoice.refundStatus} labels={INVOICE_STATUS_LABELS} />
                      </td>
                      <td className="px-5 py-3.5 text-right">
                        {invoice.pdfUrl && (
                          <a
...
function PlanCard({ plan, isCurrent, currentUsers, onSelect, isLoading }: {
  ...
  const totalPrice = plan.monthlyPriceCents * quantity;
      ...
      {hasPerSeatPricing && !isCurrent && (
        <div className="mb-4 p-3 rounded-xl bg-[rgba(255,255,255,0.03)] border border-[rgba(255,255,255,0.06)]">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-[13px] font-semibold text-[#f5f7fb]">${(totalPrice / 100).toFixed(2)}/mo</span>
          </div>
          <div className="flex items-center gap-3">
            <button
      ...
      {isCurrent ? (
        ...
        >
          {isLoading ? (
            ...
            plan.monthlyPriceCents === 0 ? t('billing.contactUs') : hasPerSeatPricing ? t('billing.subscribeUsers', { count: quantity }) : t('billing.subscribePlan')
          )}
        </button>
      )}
    </motion.div>
