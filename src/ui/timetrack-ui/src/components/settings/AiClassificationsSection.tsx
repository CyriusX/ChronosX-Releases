...
import { useAiClassifications, type ConfidenceFilter, type DecisionTypeFilter } from './useAiClassifications';
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1 min-w-0">
          ...
          {item.sampleWindowTitles && item.sampleWindowTitles.length > 0 && (
            <div className="mb-2">
              <span className="text-[10px] text-[rgba(245,247,251,0.3)]">Titulos: </span>
...
const DECISION_TYPE_TABS: { value: DecisionTypeFilter; label: string }[] = [
  ...
  { value: 'app_classification', label: 'Classificacao global' },
];
...
export function AiClassificationsSection() {
  ...
  const handleAccept = async (decisionId: string) => {
    try {
      ...
      notify.success('Classification accepted');
    } catch (err) {
  ...
  const handleReject = async (decisionId: string) => {
    try {
      ...
      notify.success('Classification rejected');
    } catch (err) {
  ...
  const handleCorrect = async (category: string, subcategory?: string) => {
    ...
    try {
      ...
      notify.success('Classification corrected');
    } catch (err) {
      notify.error(err instanceof Error ? err.message : 'Failed to correct');
    } finally {
  ...
  const handleAcceptAll = async () => {
    if (!window.confirm('Accept all suggestions with confidence >= 90%?')) return;
    try {
      const count = await acceptAll();
      notify.success(`${count} classifications auto-approved`);
    } catch (err) {
      notify.error(err instanceof Error ? err.message : 'Failed to accept all');
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-[18px] font-semibold text-[#f5f7fb]">
            {t('settings.aiClassifications.title', 'AI Classifications')}
          </h2>
          <p className="text-[12px] text-[rgba(245,247,251,0.4)]">
            {t('settings.aiClassifications.subtitle', 'Review and manage AI-generated app classification suggestions')}
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-[rgba(255,255,255,0.04)] border border-[rgba(255,255,255,0.08)]">
          ...
          <span className="text-[11px] text-[rgba(245,247,251,0.5)]">Pending</span>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12">
          ...
          <p className="text-[14px] text-[rgba(245,247,251,0.4)]">
            {t('settings.aiClassifications.empty', 'No pending classifications')}
          </p>
          <p className="text-[12px] text-[rgba(245,247,251,0.25)] mt-1">
            AI suggestions will appear here based on real team usage data
          </p>
        </div>
      ) : (
