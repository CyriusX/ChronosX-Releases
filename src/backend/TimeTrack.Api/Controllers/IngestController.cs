...
public sealed class IngestController : ControllerBase
{
    ...
    /// <param name="cancellationToken">Cancellation token</param>
    /// <returns>Processing result</returns>
    [HttpPost("focus-sessions")]
    [ProducesResponseType(typeof(IngestResponse), StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    [ProducesResponseType(StatusCodes.Status401Unauthorized)]
    [ProducesResponseType(StatusCodes.Status413PayloadTooLarge)]
    public async Task<ActionResult<IngestResponse>> IngestFocusSessions(
        [FromBody] FocusSessionIngestRequest request,
        CancellationToken cancellationToken)
    {
        // Payload size check (2MB limit)
        if (Request.ContentLength > 2 * 1024 * 1024)
        ...
        var command = new IngestFocusSessionsCommand
        {
            Items = request.Items
        };
